      ...
      <div class="mx-5 py-2">
        ...
        <div class="row flex mb-4">
          <div v-if="news" class="bg-white flex flex-wrap -mx-5">
            <div
              v-for="data in news.slice(0, 6)"
              :key="data.text"
              class="mb-6 w-full md:w-6/12 lg:w-6/12 xl:w-4/12 px-5"
            >
              <div class="">
  ...
  methods: {
    ...
    getTheGlobeNews() {
      this.news = this.$store.state.news.filter(
        (data) => data.sub_category && data.sub_category.slug === 'the-globe'
      )
    },
  },
}
